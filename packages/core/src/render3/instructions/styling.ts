...
function flushStylingOnElementExit() {
  ...
  if (classBindingIndex > 0) {
    ...
    ngDevMode && assertGreaterThan(classLastWrittenValueIndex, 1, 'There are no class bindings');
    ngDevMode &&
  ...
  if (styleBindingIndex > 0) {
    ...
    ngDevMode && assertGreaterThan(styleLastWrittenValueIndex, 1, 'There are no style bindings');
    ngDevMode &&
