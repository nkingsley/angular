...
export const instructionState: InstructionState = {
  /**
   * In:
   * - dev mode we want to set lFrame to `null` This ensures that any reads to global state will
   *   fail with error before calling `enterView`.
   * - production mode this must be set, otherwise the outer most `leviewView` would set this to
   *   `null`. This would make the whole `lView` chain eligible for garbage collection and we would
   *   lose the performance benefits of caching here.
   */
  lFrame: (typeof ngDevMode === 'undefined' || ngDevMode) ? null ! : createLFrame(null),
 ...
 *
 * The return value can be `null` if the method is called outside of template. This cane happen if
