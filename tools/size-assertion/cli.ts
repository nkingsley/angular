...
import {PathSizeMap, SizeChecker} from './size_assertion';
...
function main(argv: [string, string, string] | [string, string]): boolean {
  ...
  const goldFileContent: string = fs.readFileSync(goldenFilePath).toString();
  const expectGoldFile = JSON.parse(goldFileContent) as PathSizeMap;

  const checker = new SizeChecker(javascriptFilePath);

  let passed: boolean = false;
  if (doUpdate) {
    fs.writeFileSync(goldenFilePath, JSON.stringify(checker.actual, undefined, 2));
    console.error('Updated gold file:', goldenFilePath);
    passed = true;
  } else {
    passed = checker.assertSizeAndPrintError(expectGoldFile);
    if (!passed) {
